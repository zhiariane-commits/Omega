...
- 游戏内序章（制作人名单后）提供 API KEY 填写界面：输入视觉（火山方舟 `doubao-seed-2-0-mini-260428`）与对话（MIMO `mimo-v2.5-pro`）模型的 API KEY 后，会把密钥连同默认模型名/Base URL 写入 `game/.env.local` 并做连通性测试，成功后本次运行立即生效；失败会提示“请您换一个XX模型试一试”，可选择跳过。使用其它供应商时，在 `.env.local` 里覆盖 `*_BASE_URL`/`*_MODEL` 即可。
...
- 首次启动进入太空舱序幕。
- 序章制作人名单后新增 AI 配置步骤：填写视觉/对话模型 API KEY，应用会把密钥与默认模型/接入地址写入本地环境并执行真实连通性测试（视觉模型发送测试图片）；失败时提示更换对应模型，也可跳过以保持离线可玩。
- 昵称输入与书桌引导。
- 完成序幕后打开透明置顶悬浮窗。
