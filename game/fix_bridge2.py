...
# The browserBridge replies are fine - they already have decent Chinese.
# The main system prompt is in electron/main.ts which we already fixed.
# Let's just make the browser offline replies slightly richer.
...
for old, new in updates.items():
    if old in content:
        ...
        for i in range(len(content) - 10):
            if content[i:i+10] == old[1:11]:
                ctx = content[max(0,i-5):i+len(old)+5]
                print(f"Similar text found: {repr(ctx[:80])}")
                break
        else:
