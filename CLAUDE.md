...
- 不把下列尚未实现项当作当前迭代目标：正式美术资源与动作序列、M6「游戏」真实代打能力（仅解锁条件与占位 UI）、M7「书橱写作」完整周期（现实 3-5 天写作周期、AI 创作、故事入库书架）、正式发行与打包分发。
...
- 源文件 UTF-8；注释与文案用中文。
- 环境变量：`MIMO_API_KEY` / `MIMO_MODEL` / `MIMO_BASE_URL`（兼容 `OPENAI_*`）、`VISION_API_KEY` / `VISION_MODEL` / `VISION_BASE_URL`。模板 `game/.env.local.example` 入库跟踪，`game/.env.local` 被 gitignore 忽略、勿提交真实密钥；AI 配置也可在序章 AI 配置步骤填写并持久化到 userData `omega-ai-config.json`。
